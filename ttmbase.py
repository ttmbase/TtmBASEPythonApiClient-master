"""The module for accessing TtmBASE API written in asynchronous python"""
...
def private(func):
    ...
    A decorated function will throw an assertion error if API keys aren't placed in `ttmbase_settings.json` file.
    """
    @wraps(func)
    async def wrap(*args, **kwargs):
        assert args[0].auth is not None, "Auth not found. You can't use Account endpoints without specifying API keys. Specify \"access_key\" and \"secret_key\" in \"ttmbase_settings.json\" file."
...
class TtmBASEClient():
    """The class that for accessing TtmBASE exchange API."""
    def __init__(self, settings_file = 'ttmbase_settings.json'):
        try:
            ...
            self.auth = None
        self.endpoint = "https://ttmbase.com/api/v2"
        self.ws_endpoint = 'wss://ws.ttmbase.com'
...
# Private streams
    ...
    def ws_subscribe_reports_generator(self, ws: ClientWebSocketResponse):
        ...
        assert self.auth, "Auth not found. You can't use Account endpoints without specifying API keys. Specify \"access_key\" and \"secret_key\" in \"ttmbase_settings.json\" file."
...
# Private methods
    ...
    async def get_order(self, order_id: str):
        ...
        Args:
            order_id: TtmBASE orderId or userProvidedId
        """
